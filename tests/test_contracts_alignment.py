...
from contracts.core import __version__ as contracts_version
...
def test_canonical_id_rule_version_is_sourced_from_contracts() -> None:
    entity = make_entity()
    alias = make_alias()
    reference = make_reference()
    case = make_case()

    assert entity_registry.CANONICAL_ID_RULE_VERSION == contracts_version
    assert entity.canonical_id_rule_version == contracts_version
    assert alias.canonical_id_rule_version == contracts_version
    assert reference.canonical_id_rule_version == contracts_version
    assert case.canonical_id_rule_version == contracts_version
...
def test_internal_canonical_entity_projects_to_contract_schema() -> None:
    ...
    assert contract_entity.model_dump(mode="json") == {
        ...
        "entity_type": "stock",
        "display_name": "CATL",
        "canonical_id_rule_version": contracts_version,
...
def test_internal_entity_alias_projects_to_contract_schema() -> None:
    ...
    assert contract_alias.canonical_id_rule_version == contracts_version
...
def test_internal_resolution_case_projects_to_contract_schema() -> None:
    ...
    assert contract_case.canonical_id_rule_version == contracts_version


def test_mention_resolution_result_uses_contract_base_and_stable_shape() -> None:
    ...
    assert isinstance(result, ContractBaseModel)
